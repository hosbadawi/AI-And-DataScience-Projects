...
def task_generator(big_graph, num_tasks, days, df, ligi, on_peak, attackLocations, num_att):
    ...
    for i in range(num_tasks):
        ...
        Task_value = np.random.uniform(1,10,(num_tasks)).astype(int)  # Task Values (Uniformly distributed)
        Minute = random.randint(0,59) # Minute Range
        Hour = random.randint(1,100) # Hours Range
        
        # Hours
        if(Hour>=50):  # 50%
            h = random.randint(9, 11) # hours from 9am to 11am
        elif(Hour<=25): # 25%
            h = random.randint(12, 17) # hours from 12Pm to 5pm
        elif(Hour<50 and Hour>25): # 25%
            h = random.randint(18, 20) # hours from 6Pm to 8pm

        # Duration
        dur = random.randint(1,10)
        if(dur<6): # 50%
            duration = random.choice([20, 40, 60]) # {20, 40, 60}
        elif(dur<9 and dur<=6): # 30%
            duration = random.choice([30, 50, 70]) # {30, 50, 70}
        else: # 20%
            duration = random.choice([10, 80, 100]) # {10, 80, 100}
            

        remaining_t = duration
        resources = random.randint(7,10)
        index = random.choice(range(len(l)))
        y=big_graph.node[l[index]]['y']
        x=big_graph.node[l[index]]['x']
        grid_num=convert_location(big_graph,y,x)

        tl.append([i+1, float(y), float(x), day, h, Minute, duration, remaining_t, resources, df,ligi,on_peak,grid_num,Task_value[i]])
...
if  name_city!='no' :
   ...
   # wfile = open('punti.txt', 'w')
    ...
    for run_num in range(1,2):
        ...
        attackLo_file = open('./Inputs/Mobility/AttackLocations.txt','w')
        ...
        attackLo_file.close()
        num_tsk = NumOfTasks# int(num_usr/10)
        # num_tsk = num_usr
        ...
        '''
        # task txt file
        task_file = open('./Inputs/Mobility/Tasks.txt', 'w')
        ...
        # task txt file mytask.txt
        task_file = open('./Inputs/Mobility/mytask.txt', 'w')
        task_file.write("/ID-Task/      -/Lat/      -/Long/     -/Day/      -/Hour/     -/Minute/       -/Duration/     -/Remaining time/       -/Resources/        -/Coverage/     -/Ligitimacy/       -/on peak hour/     -/grid_number/      -/task_value\n")
        for i in range(0,len(tasks)):
        ...
        # task csv file
        csvfile = open('./Inputs/Mobility/mytask.csv','w')
        with csvfile:
            titles = ['ID','Latitude','Longitude','Day','Hour','Minute','Duration','RemainingTime','Resources','Coverage','Ligitimacy','OnPeakHours','GridNumber', 'task_value']
            writer = csv.DictWriter(csvfile, fieldnames=titles)
            writer.writeheader()
            for i in range(0,len(tasks)):
                # if(tasks[i][3]==0):
                #     continue
                writer.writerow({'ID':tasks[i][0],'Latitude':tasks[i][1],'Longitude':tasks[i][2],'Day':tasks[i][3],'Hour':tasks[i][4],'Minute':tasks[i][5],'Duration':tasks[i][6],'RemainingTime':tasks[i][7],'Resources':tasks[i][8],'Coverage':tasks[i][9],'Ligitimacy':tasks[i][10],'OnPeakHours':tasks[i][11],'GridNumber':tasks[i][12] , 'task_value':tasks[i][13]})

        #
        taskMovementl = task_movement(tasks)

        csvfile = open('./Inputs/Mobility/TasksMovement.csv','w')
        ...
        taskm_file = open('./Inputs/Mobility/TasksMovement.txt','w')
        ...
        csvfile = open('./Inputs/Mobility/Illegitimate_'+str(name_city)+str(m_r)+'.csv','w')
        with csvfile:
            titles = ['ID','Latitude','Longitude','Day','Hour','Minute','Duration','RemainingTime','Resources','Coverage','Ligitimacy','OnPeakHours','GridNumber']
            writer = csv.DictWriter(csvfile, fieldnames=titles)
            writer.writeheader()
