            ...
            for routeNum in range(number_of_routes):
                ...
                if (not social_model_routes):
                    ...
                    if stochastic_model_routes:
                       Lengths_Array = np.array(list(length.values()))
                       Lengths_Array = np.where(Lengths_Array <= 0, 0.1, Lengths_Array)
                       Weights = np.random.dirichlet(Lengths_Array , 1)
                       idr = random.choices(tuple(length), weights = Weights[0])[0]
